...
app.get('/api/events', async (req, res) => {
  try {
    ...
    // 💡 詐騙與課程過濾器
    const blacklist = ['線上 online', '付費課程', '認證班', '證照班', '說明會', '培訓', '研習', '招生'];
    ...
    const events = filtered.slice(0, 32).map((item, index) => {
      ...
      const directUrl = item.sourceWebPromote || item.webSales || `https://cloud.culture.tw/frontsite/event/eventSearchAction.do?method=doDetailView&uid=${item.uid}`;
      return {
